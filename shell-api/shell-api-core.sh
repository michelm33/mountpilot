...
###############################################################################
# 
# HUMAN-READABLE "BSA BASH SHELL API"
# 
...
# Source file last modification: 2026-05-07 20:16:57.635850609 +0000
#
    ...
    if [ ! -e "$configPath" ] ; then
        ...
        if Env__fn_exists "${appName}__${callback}" ; then
            ...
            if [ ! -e "$defaultConfigPath" ] ; then
                _log_dbg "Sumo-default config does not exist: $defaultConfigPath"
                return 4
            fi
...
@param [4] if any 4th argument is given, the raw number and unit is returned, not the computed number of bytes
